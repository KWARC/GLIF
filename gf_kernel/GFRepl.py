...
class GFRepl:
    ...
    def do_export(self, file_name):
        ...
        file_reg = re.compile('^%s' % (file_name))
        for file in files:
            _, file_extension = os.path.splitext(file)
            if file_reg.match(file) and file_extension == '.gf':
            ...
            if parse_dict['type'] == 'commands':
                for command in parse_dict['commands']:
                    ...
                    if name == 'view':
                        ...
                        messages.append(to_message_format(message=self.do_export(command['args'])))
                    elif name == 'help':
...
Stated grammars are automatically imported upon definiton.""" % (", ".join(self.to_clean_up))))
    ...
    def handle_grammar(self, grammar, name):
        ...
        with open(file_path, 'w') as f:
            f.write(grammar)
            f.close()
        out = self.handle_shell_input("import %s.gf" % (os.path.join(self.td.name,name)))
        if not out:
            out = "Defined %s" % (name)
        return out
